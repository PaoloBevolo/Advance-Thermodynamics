...
beta=((Me.evaluateProperty('beta'))*ro_fluid*cp_fluid*0); %indica il termine 
        %di trasporto che all'istante iniziale viene considerato pari a zero
        ...
        if ii > 0 
            ...
            for nj=1:3
                ...
                dtmp=c(e)*(Dy(ni)*Dy(nj)+Dx(ni)*Dx(nj))/(4.0*Areas(e)) + ...
                    (-beta(e,1)*Dy(nj)+beta(e,2)*Dx(nj))*1/6; %ho aggiunto il 
                                                %termine relativo a 'beta' 

...
MyNarginchk(nargin,4,4);
%nargoutchk(0,2);

% Error checking
if (size(p,2)~=2) || (size(t,2)~=3) || (size(Hn,2)~=1)
    error('tricontour:InvalidInputParameter','Incorrect input dimensions')
end
if size(p,1)~=size(Hn,1)
    error('tricontour:InvalidInputParameter','F and p must be the same length')
end
if (max(t(:))>size(p,1)) || (min(t(:))<=0)
    error('tricontour:InvalidInputParameter','t is not a valid triangulation of p')
end
if (size(N,1)>1) && (size(N,2)>1)
    error('tricontour:InvalidInputParameter','N cannot be a matrix')
end
        ...
        for k = 1:ncc
            ...
            if (ce==0)||(ce==start)||(flag(ce))     
                ...
                if all(flag)    % No more points at lev(v)
                    ...
                    for kedges = 1:length(edges)
                        if bnde(edges(kedges))
                            ce = edges(kedges); break
                        end
                    end
                    % Reset counters
                    start = ce;
